...
fprintf(f," %d %d %d %d %d %d %d %s   \n",co.tomate,co.carotte,co.epinard,co.pomme_de_terre,co.artichaud,co.brocoli,co.prix,co.adress);
}
fclose(f);
}
...
store=gtk_list_store_new(COLUMNS1,G_TYPE_STRING,G_TYPE_STRING,G_TYPE_STRING,G_TYPE_STRING,G_TYPE_STRING,G_TYPE_STRING,G_TYPE_STRING,G_TYPE_STRING);
 ...
 if (f== NULL){
            ...
            f=fopen("commande.txt","a+");
            while (fscanf(f,"%s %s %s %s %s %s %s %s   \n",tomate,carotte,epinard,pomme_de_terre,artichaud,brocoli,Prix,Adress)!=EOF)
{
            gtk_list_store_append(store,&iter);
            gtk_list_store_set(store,&iter,TOMATE,tomate,CAROTTE,carotte,EPINARD,epinard,POMMEDETERRE,pomme_de_terre,ARTICHAUD,artichaud,BROCOLI,brocoli,PRIX,Prix,ADRESS,Adress,-1);
